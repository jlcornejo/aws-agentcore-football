...
- Use `get_defensive_assignment` EVERY TICK to identify the most dangerous opponent to mark
- Use `calculate_pass_options` when you win the ball to find the best outlet pass
- Use `find_open_space` to position yourself optimally when tracking back

Only use tools when they add value — for simple pressing, act directly.
"""
