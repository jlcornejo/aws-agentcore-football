...
# ============================================================================
# Deploy all 5 agents WITH AgentCore Gateway (MCP tactical tools)
# ============================================================================
#
# Usage:
#   ./deploy-gateway.sh              # deploy gateway + all 5 agents
#   ./deploy-gateway.sh ai-gk        # deploy only one agent (gateway must exist)
#   GATEWAY_URL=https://... ./deploy-gateway.sh   # skip gateway creation
#
# This script:
...
#   4. Creates MCP Gateway with NONE auth (reuses if exists)
#   5. Registers Lambda targets on the gateway
#   6. Deploys all 5 agents with GATEWAY_URL env var
#   7. Attaches gateway invocation permissions to execution roles
#
# Prerequisites:
...
echo "=========================================="
echo "  ⚽ Agentic Football Cup — Gateway Deploy"
echo "=========================================="
echo ""
...
for cmd in agentcore aws rsync python3; do
...
if [ -n "$GATEWAY_URL" ]; then
  ...
  for tool in "${TOOLS[@]}"; do
    ...
    if [ ! -f "$TOOL_FILE" ]; then
      echo "  ❌ Tool file not found: $TOOL_FILE"
      exit 1
    fi
  ...
  if [ $GW_ROLE_EXISTS -ne 0 ]; then
    ...
    aws iam put-role-policy \
      ...
      --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":\"lambda:InvokeFunction\",\"Resource\":\"arn:aws:lambda:*:*:function:${LAMBDA_PREFIX}-*\"}]}"
    echo "  Waiting 10s for role propagation..."
    sleep 10
  fi
...
for AGENT in "${AGENTS[@]}"; do
  ...
  echo "=========================================="
  echo "  📦 Deploying: $AGENT (with Gateway)"
  echo "=========================================="
  ...
  # Deploy with GATEWAY_URL env var
  ...
  if (cd "$STAGE" && agentcore deploy --auto-update-on-conflict \
    ...
    echo "  ✅ $AGENT: DEPLOYED (Gateway-enabled)"
    DEPLOYED+=("$AGENT")
  else
    echo "  ❌ $AGENT: FAILED"
    FAILED+=("$AGENT")
...
echo "🏆 All agents deployed with Gateway tools!"
echo ""
echo "   Tools available to agents:"
echo "   • calculate_pass_options — Pass success probability"
echo "   • evaluate_shot          — Shot probability + aim recommendation"
echo "   • find_open_space        — Best open position by zone"
echo "   • get_defensive_assignment — Threat ranking for marking"
echo ""
echo "   Next steps:"
